...
# Explicitly set the path for your machine
# (You will need a similar line use the 'check' logic below)
if (dir.exists("C:/cmdstan/cmdstan-2.38.0")) {
  set_cmdstan_path("C:/cmdstan/cmdstan-2.38.0")
}

# Check that it's actually working (should return 2.38.0)
cmdstanr::cmdstan_version()

# Use cmdstanr for all brms models
...
m_day_95 <- brm(max_T2_95 ~ midshield_binomial + (1|div) + (1|treatment) + (1|dist_wall), 
      data = microclimate_shield_total_day, prior = priors, backend = "cmdstanr",  cores = 4, threads = threading(4), chains = 4, iter = 4000, warmup = 1000)
...
day_total_results <- process_model_group(
  ...
  backend = "cmdstanr", 
  cores = 4,
  threads = threading(4), # This uses 2 CPU cores per chain!
  iter = 4000 
)
...
make_bayesian_table <- function(model_data, title, subtitle = "Main Model (95%)") {
  ...
  # Check if model_data is a list (from our new process_model_group) 
  # or a standard model object, and extract the full tidy table
  ...
  tab <- tab_data %>%
    # Clean up hyperparameter names for the reviewer (e.g., sd__(Intercept) -> sd(Intercept))
  ...
  ft <- flextable::flextable(tab) %>%
    ...
    # Highlight Random Effect SDs so they look different from Fixed Effects
...
full_supplement_list <- list(
  "Daytime Maximum Temperature" = list(
    results = day_total_results, # This is the list containing $plot_data and $full_table
    pp_path = here("data", "figures", "pp_check_day_total.png"),
    qq_path = here("data", "figures", "pp_qq_day_total.png"), # NEW
...
iwalk(full_supplement_list, function(content, title) {
  ...
  doc <<- doc %>%
    ...
    # Pass the full_table specifically to the function
    body_add_flextable(make_bayesian_table(content$results$full_table, title))
