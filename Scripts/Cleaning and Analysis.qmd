...
## --- CmdStan Logic ---
stan_dir <- here("cmdstan")

# ONLY install if the folder is empty or missing
if (!dir.exists(stan_dir) || length(list.files(stan_dir)) == 0) {
  dir.create(stan_dir, showWarnings = FALSE)
  install_cmdstan(dir = stan_dir)
}

# Always tell R to look in that local project folder
set_cmdstan_path(stan_dir)
```
