...
priors_total <-  c(
  prior(normal(0, 0.5), class = "b", coef = "midshield_binomial"),
...
priors_daily <- c(
  prior(normal(0, 0.5), class = "b", coef = "midshield_binomial"),
  prior(normal(0, 1), class = "b", coef = "rad_scaled"),
  prior(normal(0, 1), class = "b", coef = "wind_scaled"),
  prior(normal(0, 1), class = "b", coef = "midshield_binomial:rad_scaled"),
  prior(normal(0, 1), class = "b", coef = "midshield_binomial:wind_scaled"),
  prior(normal(0, 1), class = "sd"),
  prior(normal(0, 2), class = "sigma")
)
...
m_night_05 <- brm(min_T2_05 ~ midshield_binomial + (1|div) + (1|treatment) + (1|dist_wall),  data = microclimate_shield_total_night, prior = priors_total, cores = 4, chains = 4, iter = 4000, warmup = 1000, control = list(adapt_delta = 0.999))
...
m_day_date_95 <- brm(max_T2_95 ~ midshield_binomial * (rad_scaled + wind_scaled) + (1|div) +  (1|treatment) + (1|dist_wall) + (1|Date), data = microclimate_shield_date_day, prior = priors_daily, cores = 4, chains = 4, iter = 4000, warmup = 1000, control = list(adapt_delta = 0.999))
...
m_night_date_05 <- brm(min_T2_05 ~ midshield_binomial * wind_scaled + (1|div) +  (1|treatment) + (1|dist_wall) + (1|Date), data = microclimate_shield_date_night, prior = priors_daily, cores = 4, chains = 4, iter = 4000, warmup = 1000, control = list(adapt_delta = 0.999))
...
Plot_A <- ggplot(microclimate_shield_total_day, 
  ...
  labs(x = "Shield Configuration", y = "Max Temp (°C) at 1 cm (11:00 - 17:00)") +
...
Plot_B <- ggplot(microclimate_shield_total_night, 
  ...
  stat_summary(fun = mean, geom = "point", size = 4, shape = 16) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2) +
  labs(x = "Shield Configuration", y = "Min Temp (°C) at 1 cm (23:00 - 05:00)") +
  scale_x_discrete(labels = c("present" = "Present", "absent" = "Absent")) +
  scale_color_manual(values = shield_colors) +
  scale_fill_manual(values = shield_colors) +
  theme_minimal() +
...
# Save to file
ggsave(file.path(data_path, "figure_2.jpg"), figure_2, width = 24, height = 12, dpi = 300)
...
plot_C = microclimate_shield_date_day %>%
  ...
  scale_fill_manual(values = shield_colors) +
  labs(
    x = expression("Global hourly radiation (J cm"^{-2}*")"),
...
# Save to file
ggsave(file.path(data_path, "figure_3.jpg"), figure_3, width = 24, height = 12, dpi = 300)
...
# Save to file
ggsave(file.path(data_path, "supplement_plot1.pdf"), supplement_plot1, width = 24, height = 12, dpi = 300, device = cairo_pdf)
...
Plot_A_model <- ggplot(day_pred, aes(x = midshield, y = .epred, fill = midshield, color = midshield)) +
  ...
  labs(
    ...
    y = "Max Temp (°C) at 1 cm (11:00 - 17:00)"
  ) +
  scale_x_discrete(labels = c("present" = "Present", "absent" = "Absent")) +
  scale_color_manual(values = shield_colors) +
  scale_fill_manual(values = shield_colors) +
...
Plot_B_model <- ggplot(night_pred, aes(x = midshield, y = .epred, fill = midshield, color = midshield)) +
  ...
  labs(
    x = "Shield Configuration", 
    y = "Min Temp (°C) at 1 cm (23:00 - 05:00)"
  ) +
  scale_x_discrete(labels = c("present" = "Present", "absent" = "Absent")) +
  scale_color_manual(values = shield_colors) +
  scale_fill_manual(values = shield_colors) +
...
# To save time having to run the models, load them directly from here to create plots and tables

#load rds file 

brms_day_95 <- readRDS("m_day_95.rds")

summary(brms_day_95)

brms_night_05 <- readRDS("m_night_05.rds")

summary(brms_night_05)

# Daily resolution

brms_day_date_95 <- readRDS("m_day_date_95.rds")

summary(brms_day_date_95)

brms_night_date_05 <- readRDS("m_night_date_05.rds")

summary(brms_night_date_05)
...
lmer_day_date_95 <- lmer(max_T2_95 ~ midshield_binomial * (rad_scaled + wind_scaled) + (1|div) +  (1|treatment) + (1|dist_wall) + (1|Date), data = microclimate_shield_date_day)

summary(lmer_day_date_95)

install.packages(c("performance", "see", "qqplotr"))
library(performance)

plot(lmer_day_date_95)
qqplot(lmer_day_date_95)

# Generates a 6-panel diagnostic dashboard for lmerMod
check_model(lmer_day_date_95)

# Store the check plot object
p_diag <- check_model(lmer_day_date_95)

# Save with expanded dimensions (14 x 16 inches)
ggsave("lmer_diagnostics.pdf", plot = plot(p_diag), width = 14, height = 16, dpi = 300)
