...
m_max_T_shield_date_day <- brm(
  max_T2_95 ~ midshield_binomial * yday + (1|div) + (1|dist_wall),
  prior=priors,
...
#For the total
m_max_T_shield_night = brm(max_T2_95 ~ midshield + (1|div) + (1|dist_wall), data = microclimate_shield_night)
...
Plot_ST_shield_day_date = ggplot(microclimate_shield_day, aes(x = Date, y = max_T2, color = midshield, fill = midshield)) + 
...
Plot_ST_shield_night_date = ggplot(microclimate_shield_night, aes(x = Date, y = max_T2, color = midshield, fill = midshield)) + 
  # Plot summary statistics (mean) with stat_summary
  stat_summary(fun = mean, geom = "point", size = 4, shape = 16) + 
